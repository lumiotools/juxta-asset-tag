...
void loop() {
  ...
  if (waitingForOneMinute && isFirstCycle && firstBleConnectionTracked && firstBleConnectionTime > 0) {
    ...
    if (bleMinConnectionTimeElapsed) {
      ...
      // Check if BLE is connected - if so, skip deep sleep and keep BLE running
      if (BLEConfig::isEnabled() && BLEConfig::isConnected()) {
        Serial.println("BLE is connected - skipping deep sleep, keeping BLE active");
        // Disconnect WiFi if connected (to turn off WiFi LED)
        if (CustomWiFi::isConnected()) {
          Serial.println("Disconnecting WiFi before skipping deep sleep...");
          CustomWiFi::disconnectWiFi();
        }
        // Reset cycle flags to allow next cycle
        cycleStarted = false;
        earlyBleAttempted = false;
        earlyBleSucceeded = false;
        bleStartTime = TimeSync::getCurrentTimeMillis(); // Reset BLE start time for next cycle
        // Keep BLE running and continue loop
        delay(NVSConfig::getCycleTime()*1000);
        return;
      }
      
      // BLE not connected - proceed with deep sleep
  ...
  if (canStartCycle && !cycleStarted) {
    ...
    // Keep BLE running - never turn it off
    // BLE will continue advertising and accepting connections
    if (BLEConfig::isEnabled()) {
      Serial.println("BLE advertising period complete - keeping BLE active");
      // Restart advertising if not connected to allow new connections
